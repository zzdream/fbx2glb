...
if (!target) {
  console.error("[fbx_glb][tauri-build] missing --target <triple>");
  process.exit(2);
}
...
if (clean && fs.existsSync(bundleDir)) {
  ...
  console.log(`[fbx_glb][tauri-build] cleaned: ${bundleDir}`);
}
...
if (!fs.existsSync(cliMain)) {
  console.error(`[fbx_glb][tauri-build] @tauri-apps/cli not found: ${cliMain}`);
  process.exit(2);
}
...
console.log(
  `[fbx_glb][tauri-build] ${process.execPath} ${cliMain} ${buildArgs.join(" ")}`
);
...
if (result.error) {
  console.error("[fbx_glb][tauri-build] spawn error:", result.error);
}

if (result.status !== 0 && result.status !== null) {
  console.error(`[fbx_glb][tauri-build] failed with exit code: ${result.status}`);
}

if (result.signal) {
  console.error(`[fbx_glb][tauri-build] killed by signal: ${result.signal}`);
}
